...
class TheAchievementBotApplication

fun main(args: Array<String>) {
    runApplication<TheAchievementBotApplication>(*args)
}
...
class MessageResource {
    @PostMapping("/achieve")
    fun post(@RequestBody message: AchieveMessage): Any {
    ...
    @CrossOrigin(origins = arrayOf("http://localhost:8083"))
    ...
    fun achievements(@RequestBody achieved: AchievementsMessage): Any {
        ...
        if (achieved.title != "") {
            ret = "<h1 style=\"font-family:Verdana;margin-left: 10px;color:#006600\">" + achieved.title.uppercase() + "'s achievements</h1>"
        }
    ...
    fun getBadge(achievement: Achievement): String {
        val p = 10
        val width = max((achievement.event.length * 30) + p, 300)
        val height = 150
        ...
        if (achievement.type != AchievementType.ANNIVERSARY) {
            badge += "  <text fill=\"#006600\" font-style=\"italic\" font-size=\"32\" font-family=\"Verdana\" x=\"50\" y=\"" + ((height * 0.65) + p) + "\">" + achievement.count + "X</text>"
        } else {
            badge += "  <text fill=\"#006600\" font-style=\"italic\" font-size=\"32\" font-family=\"Verdana\" x=\"50\" y=\"" + ((height * 0.65) + p) + "\">" + achievement.text + "</text>"
        }
